...
test("bundled MCP lists the generic audio toolset", async () => {
  ...
  try {
    ...
    for (const expected of ["rew_measurement_plan", "rew_repeated_session_plan", "rew_level_ladder_plan", "rew_dual_resolution_analysis", "rew_direct_late_analysis", "rew_crossover_analysis", "rew_multiseat_analysis", "rew_measurement_quality", "rew_human_listening_assessment", "audio_eq_design_plan", "audio_linked_stereo_eq_plan", "audio_speaker_protection_assessment", "audio_post_eq_verification", "audio_guided_session_plan", "audio_session_advance_plan", "audio_evidence_registry", "audio_listening_test_plan", "audio_report_plan", "rew_diagnostic_plan", "jamesdsp_preset_plan", "jamesdsp_ab_plan", "jamesdsp_ab_present_execute", "jamesdsp_ab_restore_execute", "speaker_profile_save", "audio_job_status", "audio_doctor", "rew_install_discover", "rew_launch_plan", "rew_launch_execute", "rew_capability_negotiate", "audio_artifact_validate", "audio_support_bundle_plan", "audio_dsp_apply_plan", "audio_uncertainty_budget", "audio_transfer_quality", "audio_room_metrics", "audio_speech_transmission_screening", "audio_polar_characterization", "audio_maximum_clean_output", "audio_multisource_optimize", "audio_multisource_physical_optimize", "audio_fir_design", "audio_laboratory_listening_plan", "audio_spatial_layout_assessment", "audio_sofa_metadata_assessment", "audio_evaluation_corpus_manifest", "audio_dataset_catalog", "audio_dataset_download_plan", "audio_dataset_download_execute"]) assert.ok(names.includes(expected), expected);
    ...
    const sessionPath = resolve(workspace, opened.sessionFile), qualitySession = JSON.parse(await readFile(sessionPath, "utf8")); qualitySession.currentStage = "quality-gate"; await writeFile(sessionPath, JSON.stringify(qualitySession, null, 2));
    const missingQualityArtifact = await client.callTool({ name: "audio_session_advance_plan", arguments: { sessionFile: opened.sessionFile, completedStage: "quality-gate", evidence: { accepted: true, summary: "claimed pass", artifactRefs: [] } } });
    assert.equal(missingQualityArtifact.isError, true);
