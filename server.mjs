...
import { runAnalysisWorker } from "./lib/worker-runner.mjs";
import { createEvidenceAuthority } from "./lib/evidence-tokens.mjs";

const execFileAsync = promisify(execFile);
const server = new McpServer({ name: "audio-calibration", version: "0.2.0-beta.1" });
...
const evidenceAuthority = createEvidenceAuthority(), issueEvidence = evidenceAuthority.issue, verifyEvidence = evidenceAuthority.verify;
const traceBundleHash = bundle => stableToken({ frequencyResponse: bundle?.frequencyResponse });
...
server.tool("rew_measurement_execute", "Run an exact protected sweep plan, label and save results, then restore REW state. Audible confirmation and physical readiness flags are mandatory.", {
  ...
  try {
    ...
    for (const run of p.runs) {
      ...
      const [id, summary] = await waitForMeasurement(before, measurementTimeoutMs); await rew(`/measurements/${encodeURIComponent(id)}`, { method: "PUT", body: { title: run.title, notes: JSON.stringify(runEvidence) } });
    ...
    for (const run of completed) run.traceHash = traceBundleHash(await fetchTraceBundle(run, { optionalMetrics: false }));
    const protectedEvidence = { kind: "protected-measurement-evidence", sourcePlanHash: confirmationToken, measurementFile: relative(root, p.savePath), measurementFileHash, entries: completed.map(run => ({ id: run.id, role: run.role, repeat: run.repeat, controlFingerprint: run.controlFingerprint, presetFingerprint: run.presetFingerprint, microphoneCalibrationHash: run.microphoneCalibrationHash, sweepFingerprint: run.sweepFingerprint, traceHash: run.traceHash, snrDb: run.snrDb, clipped: run.clipped })) };
    const protectedEvidenceToken = issueEvidence(protectedEvidence);
    const artifact = createCalibrationArtifact({ session: { id: confirmationToken.slice(0, 16), deviceClass: p.deviceClass, algorithmVersion: "0.2.0-beta.1", targetId: p.targetId || null, targetVersion: p.targetId ? TARGET_REGISTRY[p.targetId]?.version || null : null, mode: p.mode, sourcePlanHash: confirmationToken }, sweeps: completed.map(run => ({ id: run.id, role: run.role, repeat: run.repeat, levelDbfs: run.levelDbfs, outputChannel: run.outputChannel, fingerprints: { control: run.controlFingerprint, preset: run.presetFingerprint, microphone: run.microphoneCalibrationHash, sweep: run.sweepFingerprint }, measurementFile: relative(root, p.savePath), measurementFileHash: measurementFileHash.sha256, traceHash: run.traceHash })), provenance: { softwareVersion: "0.2.0-beta.1", rewUrl: REW_BASE, platform: process.platform, arch: process.arch } }), artifactPath = await safeWorkspacePath(root, join("sessions", `${basename(p.savePath, ".mdat")}-${confirmationToken.slice(0, 12)}.calibration.json`), [".json"]);
    await mkdir(dirname(artifactPath), { recursive: true }); await writeAtomicSet([[artifactPath, JSON.stringify(artifact, null, 2) + "\n"]], confirmationToken);
    return ok({ completed, saved: p.savePath, measurementFileHash, calibrationArtifact: artifactPath, protectedEvidenceToken, settings: p.settings, stateEvidence: { verified: true, sourcePlanHash: confirmationToken, fingerprint: p.stateFingerprint, audioIdentity: p.audioIdentity, hostState: p.hostState, jamesDspState: p.jamesDspState, monitorIntervalMs: 1500, verifiedAt: new Date().toISOString() }, skippedRequestedChannels: p.runs.map(x => x.outputChannel).filter(x => !completed.some(y => y.outputChannel === x)), next: "Run rew_measurement_quality with protectedEvidenceToken before interpretation or EQ." });
  } finally {
