...
import { bindPlan, hashFile, safeWorkspacePath, stableToken, verifyPlan } from "../core.mjs";
...
test("release tool domain validates artifacts, support redaction, jobs, negotiation, and DSP apply", async () => {
  ...
  registerReleaseTools(server, { ok, guarded: fn => fn, bindPlan, verifyPlan, stableToken, hashFile, workspaceRoot: async () => root, safeWorkspacePath, writeAtomicSet, exportFilters, rew: async path => path === "/version" ? { version: "test" } : [] });
  ...
  await mkdir(join(root,"measurements")); const mdat="immutable measurement bytes"; await writeFile(join(root,"measurements","session.mdat"),mdat); const h=(await hashFile(join(root,"measurements","session.mdat"))).sha256;
  const artifact = await handlers.get("audio_artifact_create")({ session: { id: "s", deviceClass: "general", algorithmVersion: "test", targetId: "general" }, sweeps: [{ id: "1", fingerprints: { control: stableToken("control"), preset: stableToken("preset"), microphone: stableToken("microphone"), sweep: stableToken("sweep") }, traceHash: stableToken("trace"), measurementFileHash:h, measurementFile:"measurements/session.mdat" }], analyses: {}, filters: [], provenance: { softwareVersion: "test" } });
  assert.equal((await handlers.get("audio_artifact_validate")({ artifact })).valid, true); assert.equal((await handlers.get("audio_artifact_migrate")({ artifact })).migrated, false); assert.equal((await handlers.get("audio_session_replay_validate")({ artifact })).replayable, true);
...
test("REW launch planning accepts a user executable but execution cannot bypass confirmation", async t => {
  ...
  registerReleaseTools(server, { ok, guarded: fn => fn, bindPlan, verifyPlan, stableToken, hashFile, workspaceRoot: async () => root, safeWorkspacePath, writeAtomicSet: async () => {}, exportFilters, rew: async () => { throw new Error("offline"); } });
