...
export function registerAnalysisTools(server, deps) {
  const {
    ...
    compressionMetrics, measuredPostEqVerification
  } = deps;
  ...
  server.tool("rew_measurement_quality", "Gate live REW traces on coverage, clipping, SNR, repeatability, route state, DSP state, and microphone-calibration identity.", {
    entries: z.array(liveEntrySchema).min(1).max(32), lowHz: z.number().min(5).max(1000).default(20), highHz: z.number().min(1000).max(24000).default(20000), snrDb: z.number().optional(), minSnrDb: z.number().min(5).max(60).default(15), routeStable: z.boolean().default(true), dspStable: z.boolean().default(true), stateVerified: z.boolean().default(false), expectedControlFingerprint: z.string().max(128).optional(), expectedPresetFingerprint: z.string().max(128).optional(), microphoneCalibrationHash: z.string().max(128).optional(), expectedTraceCount: z.number().int().positive().optional()
  }, guarded(async args => {
    ...
    return ok({ ...quality, fingerprintEvidence: { controlMatched, presetMatched, expectedControlFingerprint: args.expectedControlFingerprint || null, expectedPresetFingerprint: args.expectedPresetFingerprint || null } });
  }));
  ...
  server.tool("rew_human_listening_assessment", "Start a cancellable asynchronous multidimensional listening assessment; poll audio_job_status for the result.", {
    entries: z.array(liveEntrySchema).min(1).max(32), deviceClass: z.enum(["laptop", "car", "general"]), targetId: z.string().optional(), lowHz: z.number().min(5).max(1000).optional(), highHz: z.number().min(1000).max(24000).optional(), crossoverHz: z.number().min(20).max(500).optional(), snrDb: z.number().optional(), minSnrDb: z.number().min(5).max(60).optional(), routeStable: z.boolean().default(true), dspStable: z.boolean().default(true), stateVerified: z.boolean().default(false), microphoneCalibrationHash: z.string().max(128).optional()
  }, guarded(async args => {
    const job = analysisJobs.submit("rew-human-listening-assessment", async context => {
      const traces = [];
      for (const [index, entry] of args.entries.entries()) { context.throwIfCancelled(); traces.push(await fetchTraceBundle(entry)); context.progress(5 + 80 * (index + 1) / args.entries.length, `Fetched ${index + 1}/${args.entries.length} trace bundles`); }
  ...
  server.tool("audio_post_eq_verification", "Accept or reject an EQ using separately measured before/after traces, matched state fingerprints, level match, quality gates, and repeatability.", {
    beforeEntries: z.array(liveEntrySchema).min(2).max(16), afterEntries: z.array(liveEntrySchema).min(2).max(16), deviceClass: z.enum(["laptop", "car", "general"]), targetId: z.string().optional(), lowHz: z.number().min(5).max(1000).optional(), highHz: z.number().min(1000).max(24000).optional(), beforeControlFingerprint: z.string().min(8).max(128), afterControlFingerprint: z.string().min(8).max(128), beforePresetFingerprint: z.string().min(8).max(128), afterPresetFingerprint: z.string().min(8).max(128), levelMatchedWithinDb: z.number().min(0).max(12), minimumTonalImprovementDb: z.number().min(0.05).max(3).default(0.25), maximumRepeatabilityRegressionDb: z.number().min(0).max(3).default(0.25), microphoneCalibrationHash: z.string().max(128).optional()
  }, guarded(async args => {
    const [beforeTraces, afterTraces] = await Promise.all([Promise.all(args.beforeEntries.map(fetchTraceBundle)), Promise.all(args.afterEntries.map(fetchTraceBundle))]), assessmentArgs = { deviceClass: args.deviceClass, targetId: args.targetId, lowHz: args.lowHz, highHz: args.highHz, microphoneCalibrationHash: args.microphoneCalibrationHash, routeStable: true, dspStable: true, stateVerified: true }, before = humanListeningAssessment(beforeTraces, assessmentArgs), after = humanListeningAssessment(afterTraces, assessmentArgs), verification = measuredPostEqVerification(before, after, { minimumTonalImprovementDb: args.minimumTonalImprovementDb, maximumRepeatabilityRegressionDb: args.maximumRepeatabilityRegressionDb, stateMatched: args.beforeControlFingerprint === args.afterControlFingerprint && args.beforePresetFingerprint !== args.afterPresetFingerprint, levelMatchedWithinDb: args.levelMatchedWithinDb });
    return ok({ ...verification, before, after, fingerprints: { controlsMatched: args.beforeControlFingerprint === args.afterControlFingerprint, presetChanged: args.beforePresetFingerprint !== args.afterPresetFingerprint }, boundary: "Control fingerprints represent the same route, volume, microphone, geometry, and sweep settings; preset fingerprints must document the intentional DSP difference." });
  }));
}
