...
export function evaluationCorpusManifest({ synthetic = [], external = [], loopbacks = [], crossTool = [] }) {
  ...
  for (const item of candidates) {
    const domain = domains[item.domain] ||= { cataloged: 0, verifiedIndependent: 0, institutions: new Set() };
    domain.cataloged++;
    if (verifiedIndependent.includes(item)) { domain.verifiedIndependent++; for (const institution of item.institutions) domain.institutions.add(institution); }
  }
