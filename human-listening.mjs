...
export function measurementQuality(traces, { lowHz = 20, highHz = 20000, snrDb, minSnrDb = 15, requireSnr = true, routeStable = true, dspStable = true, stateVerified = false, microphoneCalibrationHash, requireMicrophoneCalibration = false, expectedTraceCount } = {}) {
...
export function humanListeningAssessment(traces, { deviceClass = "general", targetId, lowHz, highHz, crossoverHz, microphoneCalibrationHash, requireMicrophoneCalibration = false, routeStable = true, dspStable = true, stateVerified = false, snrDb, minSnrDb, requireSnr = true } = {}) {
...
function comparisonChartSvg(comparison) {
  const groups = (comparison?.groups || []).map((group, index) => ({ ...group, color: group.color || ["#d97706", "#0891b2", "#7c3aed", "#dc2626"][index % 4], rows: (group.rows || []).filter(row => Number.isFinite(row.frequencyHz) && Number.isFinite(row.meanDb) && row.frequencyHz > 0) })).filter(group => group.rows.length);
  if (groups.length < 2) return "";
  const all = groups.flatMap(group => group.rows), minF = Math.min(...all.map(row => row.frequencyHz)), maxF = Math.max(...all.map(row => row.frequencyHz)), minDb = Math.floor(Math.min(...all.map(row => Number.isFinite(row.lowDb) ? row.lowDb : row.meanDb)) - 2), maxDb = Math.ceil(Math.max(...all.map(row => Number.isFinite(row.highDb) ? row.highDb : row.meanDb)) + 2);
  const width = 1000, height = 520, left = 68, right = 26, top = 74, bottom = 58, x = frequency => left + Math.log(frequency / minF) / Math.log(maxF / minF) * (width - left - right), y = db => top + (maxDb - db) / (maxDb - minDb || 1) * (height - top - bottom);
  const gridF = [20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000].filter(frequency => frequency >= minF && frequency <= maxF), gridDb = Array.from({ length: 7 }, (_, index) => minDb + index * (maxDb - minDb) / 6);
  const band = group => { const upper = group.rows.map(row => `${round(x(row.frequencyHz), 1)},${round(y(Number.isFinite(row.highDb) ? row.highDb : row.meanDb), 1)}`), lower = [...group.rows].reverse().map(row => `${round(x(row.frequencyHz), 1)},${round(y(Number.isFinite(row.lowDb) ? row.lowDb : row.meanDb), 1)}`); return [...upper, ...lower].join(" "); };
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Measured before and after frequency-response comparison"><rect width="${width}" height="${height}" rx="18" fill="#0f1b2d"/><text x="${left}" y="38" fill="#f8fafc" font-size="24" font-weight="700">${escapeHtml(comparison.title || "Before / After")}</text><text x="${left}" y="60" fill="#9fb0c5" font-size="13">${escapeHtml(comparison.subtitle || "Repeated REW measurements · averaged response · shaded ±1 SD")}</text>${gridF.map(frequency => `<line x1="${x(frequency)}" y1="${top}" x2="${x(frequency)}" y2="${height - bottom}" stroke="#94a3b8" stroke-opacity=".17"/><text x="${x(frequency)}" y="${height - 25}" text-anchor="middle" fill="#9fb0c5" font-size="12">${frequency >= 1000 ? `${frequency / 1000}k` : frequency}</text>`).join("")}${gridDb.map(db => `<line x1="${left}" y1="${y(db)}" x2="${width - right}" y2="${y(db)}" stroke="#94a3b8" stroke-opacity=".17"/><text x="${left - 9}" y="${y(db) + 4}" text-anchor="end" fill="#9fb0c5" font-size="12">${round(db, 1)}</text>`).join("")}${groups.map(group => `<polygon points="${band(group)}" fill="${group.color}" opacity=".13"/><polyline points="${group.rows.map(row => `${round(x(row.frequencyHz), 1)},${round(y(row.meanDb), 1)}`).join(" ")}" fill="none" stroke="${group.color}" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>`).join("")}<g transform="translate(${Math.max(left, width - right - 285)} 22)">${groups.map((group, index) => `<line x1="0" y1="${index * 22}" x2="30" y2="${index * 22}" stroke="${group.color}" stroke-width="3"/><text x="40" y="${index * 22 + 5}" fill="#eef5ff" font-size="14" font-weight="600">${escapeHtml(group.label)}</text>`).join("")}</g><text x="${left + (width - left - right) / 2}" y="${height - 8}" text-anchor="middle" fill="#bdc9d8" font-size="14">Frequency (Hz) · logarithmic scale</text><text x="18" y="${top + (height - top - bottom) / 2}" text-anchor="middle" fill="#bdc9d8" font-size="14" transform="rotate(-90 18 ${top + (height - top - bottom) / 2})">Level (dB SPL)</text></svg>`;
}
export function renderHumanReport({ title = "Audio Calibration Report", assessment, eq, listening, resolutionViews, comparisonViews } = {}) {
  ...
  if (comparisonViews) lines.push("", "## Before / after comparison", "", "Repeated traces are averaged per state. The standalone SVG and HTML overlay show the mean response with a shaded ±1 standard-deviation band.", "", comparisonViews.measuredLevel ? `Measured after-minus-before level difference: ${comparisonViews.measuredLevel.differenceDb} dB across ${comparisonViews.measuredLevel.bandHz[0]}–${comparisonViews.measuredLevel.bandHz[1]} Hz.` : "");
  ...
  const markdown = lines.join("\n") + "\n", chart = resolutionChartSvg(resolutionViews), comparisonSvg = comparisonChartSvg(comparisonViews), html = `<!doctype html><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{font:16px system-ui;max-width:1000px;margin:2rem auto;padding:0 1rem;line-height:1.5}li{margin:.5rem 0}figure{margin:1.5rem 0}figure svg{width:100%;height:auto}figcaption{font-size:.9rem;margin-top:.5rem}</style><body>${lines.map(line => line.startsWith("# ") ? `<h1>${escapeHtml(line.slice(2))}</h1>` : line.startsWith("## ") ? `<h2>${escapeHtml(line.slice(3))}</h2>${line === "## Resolution views" ? chart : line === "## Before / after comparison" && comparisonSvg ? `<figure>${comparisonSvg}<figcaption>Mean response with shaded ±1 SD across repeated traces.</figcaption></figure>` : ""}` : line.startsWith("- ") ? `<li>${escapeHtml(line.slice(2))}</li>` : line ? `<p>${escapeHtml(line)}</p>` : "").join("\n")}</body>`;
  return { markdown, html, comparisonSvg: comparisonSvg || null, json: { schemaVersion: 2, title, assessment, eq: eq || null, listening: listening || null, resolutionViews: resolutionViews || null, comparisonViews: comparisonViews || null } };
}
